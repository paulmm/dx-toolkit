...
public class DXTrimReads {
    ...
    private static class GTableGetInput {
        @JsonProperty
        private int starting;
        @JsonProperty
        private int limit;

        public GTableGetInput(int starting, int limit) {
    ...
    @JsonIgnoreProperties(ignoreUnknown = true)
    private static class GTableGetOutput {
        @JsonProperty
        private List<ArrayNode> data;
    }

    private static class GTableAddRowsInput {
        @JsonProperty
        private int part;
        @JsonProperty
        private List<ArrayNode> data;

        public GTableAddRowsInput(int part, List<ArrayNode> data) {
            this.part = part;
            this.data = data;
        }
    }
    ...
    public static void main(String[] args) throws IOException {
        ...
        ObjectNode gtableNewInput = DXJSON.getObjectBuilder()
            .put("initializeFrom",
                 ...
                 .build())
            .build();
        DXGTable trimmedReads = DXGTable.getInstance(DXAPI.gtableNew(gtableNewInput, JsonNode.class).get("id").textValue());
        ...
        for (int i = 0; i < gtableDesc.getNumRows(); i += step) {
            GTableGetInput gtableGetInput = new GTableGetInput(i, step);
            List<ArrayNode> inputRows = DXAPI.gtableGet(readsTable.getId(), gtableGetInput, GTableGetOutput.class).data;
            ...
            DXAPI.gtableAddRows(trimmedReads.getId(), new GTableAddRowsInput(nextPartIndex++, outputRows), JsonNode.class);
        }
